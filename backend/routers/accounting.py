...
async def get_cong_trinh():
    ...
    try:
        ...
        for item in result.data:
            transformed_item = {
                'id': item['id'],
                'name_congtrinh': item.get('name_congtrinh', ''),
                'name_customer': item.get('name_customer', ''),
                'Id_sale': item.get('Id_sale', ''),
                'dia_chi': item.get('dia_chi', ''),
                'so_dien_thoai': item.get('sdt', ''),
                'email': item.get('email', ''),
                'ngay_bat_dau': None,  # Not in new structure
                'ngay_hoan_thanh': None,  # Not in new structure
                'ngan_sach_du_kien': item.get('ngan_sach_du_kien'),
                'mo_ta': item.get('mo_ta', ''),
                'status': item.get('status', ''),
                'created_at': item.get('created_at'),
                'updated_at': item.get('created_at')  # No updated_at in new structure
            }
            transformed_data.append(transformed_item)
...
async def create_cong_trinh(cong_trinh_data: dict):
    ...
    try:
        ...
        result = supabase.table('cong_trinh').insert({
            'name_congtrinh': cong_trinh_data.get('ten_cong_trinh'),  # Map from frontend
            'dia_chi': cong_trinh_data.get('dia_chi'),
            'name_customer': cong_trinh_data.get('khach_hang'),  # Map from frontend
            'sdt': safe_numeric(cong_trinh_data.get('so_dien_thoai')),
            'email': cong_trinh_data.get('email'),
            'Id_sale': safe_int(cong_trinh_data.get('id_sale')),
            'ngan_sach_du_kien': safe_numeric(cong_trinh_data.get('ngan_sach_du_kien')),
            'mo_ta': cong_trinh_data.get('mo_ta'),
            'status': cong_trinh_data.get('status', 'báo giá')
        }).execute()
...
async def update_cong_trinh(cong_trinh_id: int, cong_trinh_data: dict):
    ...
    try:
        ...
        result = supabase.table('cong_trinh').update({
            ...
            'ngan_sach_du_kien': safe_numeric(cong_trinh_data.get('ngan_sach_du_kien')),
            'mo_ta': cong_trinh_data.get('mo_ta'),
            'status': cong_trinh_data.get('status')
