#!/usr/bin/env python3
"""
Script to clear existing payroll data and add fresh sample data
"""
import sys
import os
...
try:
    ...
    print("Clearing salary slips...")
    supabase.table('phieu_luong').delete().neq('id', 0).execute()

    print("Clearing product salary data...")
    supabase.table('luong_san_pham').delete().neq('id', 0).execute()

    print("Clearing timesheets...")
    ...
    print("\n📝 Adding fresh sample payroll data...")
    ...
    print("\n🎉 Fresh sample payroll data added successfully!")
    print(f"👥 {len(sample_employees)} employees")
    print(f"📅 {len(sample_timesheets)} timesheets")
    print(f"📦 {len(sample_products)} product salary records")
    print(f"\n🚀 Ready to test payroll management features!")
