...
user_result = supabase.table('users').select('*').eq('email', 'levanc@company.com').execute()
if user_result.data:
    ...
    users_result = supabase.table('users').select('id, full_name, email').limit(5).execute()
